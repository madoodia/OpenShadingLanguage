...
public:
  ...
  Magnifier(QWidget *parent, int res = 170)
      : QWidget(parent), m_image(nullptr), m_info(nullptr), m_renderview(nullptr), m_res(res)
  {
    int textSize = 100;
...
void OSLToyMainWindow::createActions()
{
...
void OSLToyMainWindow::createMenus()
{
...
void OSLToyMainWindow::createStatusBar()
{
...
int OSLToyMainWindow::ntabs() const
{
  int n = textTabs->count();
  return n;
}

void OSLToyMainWindow::update_statusbar_fps(float time, float fps)
{
  statusFPS->setText(
      OIIO::Strutil::sprintf("  %.2f    FPS: %5.1f", time, fps).c_str());
}

void OSLToyMainWindow::clear_param_area()
{
  if (paramScroll)
  {
...
void OSLToyMainWindow::action_newfile()
{
...
void OSLToyMainWindow::action_open()
{
...
bool OSLToyMainWindow::open_file(const std::string &filename)
{
  std::string contents;
  ...
  bool current_tab_empty = (texteditor->blockCount() == 1) && texteditor->text_string().empty();
...
void OSLToyMainWindow::action_saveas()
{
  int tab = textTabs->currentIndex();
  CodeEditor *texteditor = editors[tab];
...
void OSLToyMainWindow::action_save()
{
  int tab = textTabs->currentIndex();
  CodeEditor *texteditor = editors[tab];
...
void OSLToyMainWindow::timed_rerender_trigger(void)
{
  if (paused)
    return;
  float now = timer();
...
void OSLToyMainWindow::osl_do_rerender(float /*frametime*/)
{
  using namespace OIIO;
  m_rerender_needed = 0;
...
void OSLToyMainWindow::toggle_pause()
{
  OIIO::spin_lock lock(m_job_mutex);
  if (paused)
  {
...
void OSLToyMainWindow::restart_time()
{
  OIIO::spin_lock lock(m_job_mutex);
  timer.reset();
  timer.start();
...
void OSLToyMainWindow::build_shader_group()
{
...
void OSLToyMainWindow::inventory_params()
{
  ShadingSystem *ss = renderer()->shadingsys();
...
void OSLToyMainWindow::make_param_adjustment_row(ParamRec *param,
...
void OSLToyMainWindow::set_ui_to_paramval(ParamRec *param)
{
...
void OSLToyMainWindow::reset_param_to_default(ParamRec *param)
{
  // Erase the instance value override
...
void OSLToyMainWindow::set_param_diddle(ParamRec *param, int diddle)
{
...
void OSLToyMainWindow::set_param_instance_value(ParamRec *param)
{
...
void OSLToyMainWindow::rebuild_param_area()
{
  ShadingSystem *ss = renderer()->shadingsys();
  ShaderGroupRef group = renderer()->shadergroup();
  if (!group)
...
void OSLToyMainWindow::set_error_message(int tab, const std::string &msg)
{
...
void OSLToyMainWindow::finish_and_close()
{
...
void OSLToyMainWindow::mousePressEvent(QMouseEvent *event)
{
#if 0
